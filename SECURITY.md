...
- Credentials (`MAILRU_USERNAME`, `MAILRU_APP_PASS`) are injected strictly via environment variables at process spawn time (e.g. from `.env` with `chmod 600` or a secure secret vault).
...
- Execution requires explicit confirmation from the human operator followed by a call to `execute_pending_action(token)`. Replay attacks are prevented by atomically popping and invalidating tokens upon consumption.

### 4. Path Traversal & Workspace Containment Defense
- The `dav_download_file`, `dav_upload_file`, and `mail_send_with_attachment` tools strictly validate local target/source paths using `webdav.ValidateDownloadPath()`.
- File operations are strictly confined to authorized roots (user home, CWD, system temporary directory, or custom paths specified via `MAILRU_ALLOWED_ROOTS`).
- Direct or indirect attempts to escape into unauthorized system directories (`/etc`, `~/.ssh`, `/var`, `/proc`) are halted with a security violation.
